...
def analyze_route(req: AnalyzeRequest):
    ...
    base_cost_per_km = 45 if req.transport_mode.lower() == "road" else 30
    distance = result["primary_route"].get("distance_km") or 0
    base_cost = round(distance * base_cost_per_km, 0)

    alternate_distance = None
    if result["alternate_route"]:
        alternate_distance = result["alternate_route"].get("distance_km")
    ...
    return {
        ...
        "mitigation": [
            {
                "option": "Use recommended route",
                "detail": f"via {primary_hw}",
                "time_impact": f"+{expected_delay_days} days",
                "cost_impact": f"₹{base_cost:,.0f} estimated"
            },
            {
                ...
                "time_impact": f"{result['alternate_route']['duration_hours']}hrs"
                               if result["alternate_route"] else "N/A",
                "cost_impact": f"₹{alternate_distance * base_cost_per_km:,.0f}"
                               if alternate_distance else "N/A"
            },
            {
